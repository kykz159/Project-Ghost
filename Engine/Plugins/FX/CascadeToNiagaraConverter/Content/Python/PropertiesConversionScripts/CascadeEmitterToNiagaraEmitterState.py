    ...
    def convert(
        ...
        if num_emitter_loops == 0:
            ...
            loop_behavior_input = ueFxUtils.create_script_input_enum(
                Paths.enum_niagara_emitter_state_options,
                "Multiple")
            loop_count_input = ueFxUtils.create_script_input_int(num_emitter_loops)
            emitter_state_script.set_parameter("Loop Count", loop_count_input)
        ...
        if emitter_origin != ue.Vector.ZERO:
            mode_input = ueFxUtils.create_script_input_enum(Paths.enum_niagara_position_initialization_mode, "Simulation Position")
            initialize_script.set_parameter("Position Mode", mode_input)
            use_offset_input = ueFxUtils.create_script_input_bool(True)
            initialize_script.set_parameter("UsePositionOffset", use_offset_input)
            pos_offset_input = ueFxUtils.create_script_input_vector(emitter_origin)
            initialize_script.set_parameter("Position Offset", pos_offset_input)
            coordinate_space_input = ueFxUtils.create_script_input_enum(Paths.enum_niagara_coordinate_space, "Local")
            initialize_script.set_parameter("Position Offset Coordinate Space", coordinate_space_input)

        script_args = ue.CreateScriptContextArgs(ueFxUtils.create_asset_data(Paths.script_particle_state), [1, 1])
        niagara_emitter_context.find_or_add_module_script("ParticleState", script_args, ue.ScriptExecutionCategory.PARTICLE_UPDATE)
