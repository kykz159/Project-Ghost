...
class SpriteConverter(RendererConverterInterface):
    ...
    def convert(cls, cascade_typedata_module, cascade_required_module,
        ...
        (material_interface, screen_alignment, subimages_horizontal, 
         ...
         ) = ueFxUtils.get_particle_module_required_per_renderer_props(cascade_required_module)

        sprite_renderer_props.set_editor_property(
            "Material", material_interface)

        sprite_alignment, sprite_facingmode = c2nUtils.get_alignment_and_facingmode_for_screenalignment(
            screen_alignment)
        ...
        niagara_sort_mode, sort_binding = c2nUtils.get_niagara_sortmode_and_binding_for_sortmode(sort_mode)
        ...
        subimage_blend, subimage_binding = c2nUtils.get_enable_subimage_blend_and_binding_for_interpolation_method(
        ...
        if subimage_binding is not None:
            ...
            niagara_emitter_context.log(
                "Tried to set subimage binding for sprite renderer but custom subimage binding conversion is not "
                "supported.",
                ue.NiagaraMessageSeverity.WARNING)
            
            # sprite_renderer_props.set_editor_property("SubImageIndexBinding", subimage_binding)

        sprite_renderer_props.set_editor_property("bRemoveHMDRollInVR", b_remove_hmd_roll)
        ...
        sprite_renderer_props.set_editor_property("MinFacingCameraBlendDistance", min_facing_camera_blend_distance)

        sprite_renderer_props.set_editor_property("MaxFacingCameraBlendDistance", max_facing_camera_blend_distance)

        if material_cutout_texture is not None:
            sprite_renderer_props.set_editor_property("bUseMaterialCutoutTexture", False)
            sprite_renderer_props.set_editor_property("CutoutTexture", material_cutout_texture)

        sprite_renderer_props.set_editor_property("BoundingMode", bounding_mode)

        sprite_renderer_props.set_editor_property("OpacitySourceMode", opacity_source_mode)
        
        if normal_facing_mode != ue.EmitterNormalsMode.ENM_CAMERA_FACING:
            niagara_emitter_context.log(
                "Failed to copy sprite renderer \"Emitter Normal Mode\": Niagara sprite renderer does not support "
                "this.",
                ue.NiagaraMessageSeverity.ERROR)
        
        # set the opacity alpha threshold
        sprite_renderer_props.set_editor_property("AlphaThreshold", alpha_threshold)

        # add the renderer properties
        niagara_emitter_context.add_renderer("SpriteRenderer", sprite_renderer_props)
