...
class CascadeMeshTypeDataConverter(RendererConverterInterface):
    ...
    def convert(cls, cascade_typedata_module, cascade_required_module,
        ...
        (mesh, lod_size_scale, b_use_static_mesh_lods, b_cast_shadows,
         ...
         ) = ueFxUtils.get_particle_module_type_data_mesh_props(cascade_typedata_module)
        ...
        if screen_alignment is ue.ParticleScreenAlignment.PSA_TYPE_SPECIFIC:
            if mesh_alignment is not ue.MeshScreenAlignment.PSMA_MESH_FACE_CAMERA_WITH_LOCKED_AXIS:
                mesh_facing_mode = ue.NiagaraMeshFacingMode.CAMERA_POSITION
                mesh_renderer_props.set_editor_property("FacingMode", mesh_facing_mode)

        if screen_alignment is ue.ParticleScreenAlignment.PSA_VELOCITY:
            mesh_facing_mode = ue.NiagaraMeshFacingMode.VELOCITY
            mesh_renderer_props.set_editor_property("FacingMode", mesh_facing_mode)
