...
class CascadeSpawnConverter(ModuleConverterInterface):
    ...
    def convert(cls, args):
        ...
        if b_process_spawn_burst is True:
            cls.convert_spawn_burst(burst_scale_distribution, burst_method, burst_list, emitter, typedata_module)
    ...
    def convert_spawn_rate(
        ...
        if (rate_scale_distribution is not None) and c2nUtils.distribution_always_equals(rate_scale_distribution, 1.0) is False:
    ...
    def convert_spawn_burst(
        ...
        for burst in burst_list:
            ...
            if isinstance(typedata_module, ue.ParticleModuleTypeDataBeam2):
                count = max(count, 2)
            ...
            if c2nUtils.distribution_always_equals(burst_scale_distribution, 1.0) is False:
                ...
                integer_conversion_mode_input = ueFxUtils.create_script_input_enum(Paths.enum_niagara_integer_conversion_mode, "Ceil")
                int_from_float_script.set_parameter("Conversion Mode", integer_conversion_mode_input)
                count_input = ueFxUtils.create_script_input_dynamic(
