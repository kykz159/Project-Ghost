...
class CascadeWorldOffsetConverter(ModuleConverterInterface):
    ...
    def get_input_cascade_module(cls):
        return ue.ParticleModuleLocationWorldOffset
    ...
    def convert(cls, args):
        ...
        (start_location_distribution,
         ...
         ) = ueFxUtils.get_particle_module_location_world_offset_props(cascade_module)
        ...
        if distribute_over_n_points != 0.0 and distribute_over_n_points != 1.0:
            ...
            options = c2nUtils.DistributionConversionOptions()
            options.set_custom_indexer(indexer_input)
            position_input = c2nUtils.create_script_input_for_distribution(start_location_distribution)
        ...
        coordinate_space_input = ueFxUtils.create_script_input_enum(Paths.enum_niagara_coordinate_space, "World")
        di_transform_vector_script.set_parameter("Source Space", coordinate_space_input)
        ...
        add_vector_to_position_script = emitter.find_or_add_module_script(
            "AddVectorToPosition_World" + str(cls.call_count),
            script_args,
