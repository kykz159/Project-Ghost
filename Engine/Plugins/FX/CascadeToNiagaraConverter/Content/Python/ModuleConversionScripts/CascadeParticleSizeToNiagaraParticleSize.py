...
class CascadeParticleSizeConverter(ModuleConverterInterface):
    ...
    def convert(cls, args):
        ...
        elif type(typedata_module) == ue.ParticleModuleTypeDataBeam2:
            # noinspection PyTypeChecker
            cls.__convert_ribbon_size(cascade_module, emitter_context, True)
    ...
    @classmethod
    def __convert_ribbon_size(cls, cascade_module, emitter_context, is_beam=False):
        """
        ...
        Args:
            ...
            emitter_context (ue.NiagaraEmitterConversionContext): 
        """
        script_args = ue.CreateScriptContextArgs(ueFxUtils.create_asset_data(Paths.script_initialize_particle), [1, 0])
        initialize_particle_script = emitter_context.find_or_add_module_script(
            "InitializeParticle", 
            script_args,
            ue.ScriptExecutionCategory.PARTICLE_SPAWN)
        ...
        if is_beam:
            ...
            b_value = 2.0
        scale_ribbon_script.set_parameter("B", ueFxUtils.create_script_input_float(b_value))
        size_input = ueFxUtils.create_script_input_dynamic(scale_ribbon_script, ue.NiagaraScriptInputType.FLOAT)
        
        # set the size mode and size
        size_mode_input = ueFxUtils.create_script_input_enum(Paths.enum_niagara_unset_directset, "Direct Set")
        initialize_particle_script.set_parameter("Ribbon Width Mode", size_mode_input)
        initialize_particle_script.set_parameter("Ribbon Width", size_input)
