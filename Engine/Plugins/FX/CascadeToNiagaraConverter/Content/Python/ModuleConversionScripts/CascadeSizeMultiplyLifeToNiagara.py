...
class CascadeSizeMultiplyLifeConverter(ModuleConverterInterface):
    ...
    def __convert_sprite_size(cls, cascade_module, required_module, emitter):
        ...
        # Niagara sprite size module's "Uniform" refers to the uniformity of the X and Y components.
        # Assuming a uniform scale to start.
        scale_mode_input = ueFxUtils.create_script_input_enum(Paths.enum_niagara_scale_sprite_size, "Uniform")
        scale_factor_parameter_name = "Uniform Scale Factor"

        # if multiplying through all components, do it normally.
        # Assuming a uniform scale to start, a Float is used.
        if multiply_x and multiply_y:
            ...
            # scale uniformly across both axes.
                ...
                # In this case the X and Y components are actually different, so a non-uniform scale is needed.
                # This will be represented as a 2D Vector, instead of Cascade's 3D Vector, since sprites are 2D.
                options.set_target_type_width(ue.NiagaraScriptInputType.VEC2)
                scale_mode_input = ueFxUtils.create_script_input_enum(Paths.enum_niagara_scale_sprite_size, "Non-Uniform")
                scale_factor_parameter_name = "Scale Factor"
            
            life_multiplier_input = c2nUtils.create_script_input_for_distribution(life_multiplier, options)

            # Niagara's scale sprite size module has different modes that accepts different inputs.
            # "Scale Sprite Size Mode" determines if the scale is uniformly applied i.e. the same value for X and Y,
            # or if the scale is non-uniform and a 2D vector should represent X and Y independently.
            # The Scale Factor's name will change depending on the Size Mode. "Uniform Scale Factor" for uniform modes
            # and "Scale Factor" for non-uniform modes.
            scale_sprite_script.set_parameter("Scale Sprite Size Mode", scale_mode_input)
            scale_sprite_script.set_parameter(scale_factor_parameter_name, life_multiplier_input)

            # NOTE: The Niagara module has modes for curves or constant values, which doesn't map well to Cascade's
            # distributions, so a dynamic input will be used to make them compatible.
