...
class CascadeCollisionConverter(ModuleConverterInterface):
    ...
    def convert(cls, args):
        ...
        script_asset = ueFxUtils.create_asset_data(Paths.script_collision)
        script_args = ue.CreateScriptContextArgs(script_asset)
        collision_script = emitter.find_or_add_module_script(
            "Collision",
            script_args,
        ...
        (damping_factor, rotation_damping_factor, max_collisions,
         ...
         ) = ueFxUtils.get_particle_module_collision_props(cascade_module)
