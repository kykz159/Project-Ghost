...
class CascadeColorScaleConverter(ModuleConverterInterface):
    ...
    def convert(cls, args):
        ...
        # If converting with a color over life module, scale based on those vales otherwise they would be overridden
        color_over_life_module = emitter.find_module_script("ColorOverLife")
        if color_over_life_module is not None and color_over_life_module.get_module_enabled():
            color_value_input = ueFxUtils.create_script_input_linked_parameter(
                "Particles.Color",
                ue.NiagaraScriptInputType.LINEAR_COLOR
            )
