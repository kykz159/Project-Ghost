...
def train(training_model, create_network_function) -> int:
    """Save the neural morph network into its own file format.
    The data saved in this file is read after training, to create setup a neural network with custom inference.
    This is specific to the Neural Morph Model, to get maximum performance at runtime.
    ...
    print('MLDeformer has Tensorboard:', has_tensorboard)
    print('MLDeformer Dev Mode:', dev_mode)   
    ...
    num_curve_values = num_curves * num_floats_per_curve
    ...
    with ue.ScopedSlowTask(4, 'Preparing for training') as training_task:
        ...
        mask_data_list = [training_model.get_training_input_anim_mask_data(mask_name) for mask_name in mask_names]        
        input_anim_mask_tensor = torch.tensor(mask_data_list, dtype=torch.float32, device=device).repeat_interleave(3, dim=1)             
    ...
    try:
            ...
            for i in range(num_iters):
                ...
                if dev_mode:
                    ...
                    if i % 5 == 0:
                        ...
                        if has_tensorboard and tensorboard_writer:
        ...
        if dev_mode and has_tensorboard and tensorboard_writer:
            tensorboard_writer.close()

        # Save the trained network to a file.
        model_class_name = str(training_model.get_model().get_class().get_fname())
        use_onnx = False
        if use_onnx:
            network.eval()
            if platform.system() != "Linux":
                ...
                torch.onnx.export(
                    network, x[:1],
                    os.path.join(training_dir, model_class_name + '.onnx'),
                    verbose=False,
                    export_params=True,
                ...
                training_state = 'aborted'
                model_path = os.path.join(training_dir, model_class_name + '.pt')
                model_dict_path = os.path.join(training_dir, model_class_name + '_StateDict.pth')
            ...
            save_neural_morph_network(
                network=network,
                filename=os.path.join(training_dir, model_class_name + '.nmn'),
