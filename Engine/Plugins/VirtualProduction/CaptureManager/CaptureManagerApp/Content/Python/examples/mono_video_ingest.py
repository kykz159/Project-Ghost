...
from capture.devices import MonoVideoIngestDevice
from capture.ingest import ingest_takes, is_from_today
...
def main():
    parser = argparse.ArgumentParser(description='Example Mono Video Ingest device')
    parser.add_argument('--archive-path', required=True, type=str, help='Path to the mono video')
    parser.add_argument('--video-discovery-expression', type=str, default='<Auto>', help='Format of the video files')
    ...
    with UnrealEndpointManager(connect_timeout=10) as endpoint_manager:
        ...
        with MonoVideoIngestDevice(args.archive_path, args.video_discovery_expression) as mono_video:
            ingest_takes(mono_video, settings)
