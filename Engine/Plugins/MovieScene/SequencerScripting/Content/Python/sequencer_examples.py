...
def render_sequence_to_movie(sequencer_asset_path):
	...
	burn_in_options.settings.set_editor_property('TopCenterText', "{RootName} - {Date} - {EngineVersion}")
	...
	burn_in_options.settings.set_editor_property('BottomCenterText', "{hh}:{mm}:{ss}:{ff} ({RootFrame})")
...
def sequence_to_dict(sequence):
	...
	for track in sequence.find_tracks_by_type(unreal.MovieSceneTrack):
		d['tracks'].append(track_to_dict(track))
...
'''
	Summary:
		Creates a root level sequence with the given name under the specified directory. This is an example of how to create Level Sequence assets,
		how to add objects from the current map into the sequence and how to create some example bindings/etc. Assumes you have a default level
	...
	Returns:
		The created LevelSequence asset.
'''
def create_root_level_sequence(asset_name, package_path = '/Game/', num_subsequences = 1, length_seconds = 5):

	root_level_sequence = unreal.AssetToolsHelpers.get_asset_tools().create_asset(asset_name, package_path, unreal.LevelSequence, unreal.LevelSequenceFactoryNew())
	cinematic_shot_track = root_level_sequence.add_track(unreal.MovieSceneCinematicShotTrack)
	...
	for i in range(num_subsequences):
		...
		camera_binding_id = subsequence.get_binding_id(camera_binding)
		camera_cut_section.set_camera_binding_id(camera_binding_id)

	return root_level_sequence
...
def create_level_sequence_with_mpc(mpc_asset_path, mpc_parameter_name, asset_name, package_path = '/Game/'):
	...
	mpc_track = sequence.add_track(unreal.MovieSceneMaterialParameterCollectionTrack)
