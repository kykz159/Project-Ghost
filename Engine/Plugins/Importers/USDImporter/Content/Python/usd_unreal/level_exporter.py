...
def assign_ism_component_assets(component, prim, context, material_overrides_layer, converter):
    ...
    proxy_prim.GetReferences().AddReference(exported_filepath)
...
def convert_component(context, converter, component, visited_components=set(), parent_prim_path=""):
    ...
    if isinstance(component, unreal.SceneComponent):
        ...
        if isinstance(owner_actor, unreal.InstancedFoliageActor):
            ...
            if success:
                prim.ClearTypeName()
                prim.GetReferences().AddReference(mesh_path)
            else:
...
def export_material(context, material):
    ...
    """
    material_file = unreal.UsdConversionLibrary.get_unique_file_path_for_export(
        os.path.join(context.options.inner.asset_folder.path, material.get_name() + context.scene_file_extension)
    )
...
def export_mesh(context, mesh, mesh_type):
    ...
    :returns: (Bool, String) containing True if the export was successful, and the output filename that was used
    """
    mesh_file = unreal.UsdConversionLibrary.get_unique_file_path_for_export(
        os.path.join(context.options.inner.asset_folder.path, mesh.get_name() + context.scene_file_extension)
    )
...
def export_cubemap(context, cubemap):
    ...
    hdr_file = unreal.UsdConversionLibrary.get_unique_file_path_for_export(
        os.path.join(context.options.inner.asset_folder.path, "Textures", cubemap.get_name() + ".hdr")
    )
    ...
    with unreal.ScopedSlowTask(num_cubemaps, 'Exporting/copying cubemaps') as slow_task:
        ...
        for cubemap in cubemaps:
            ...
            if os.path.exists(existing_filename) and os.path.isfile(existing_filename):
                target_filename = unreal.UsdConversionLibrary.get_unique_file_path_for_export(
                    os.path.join(context.options.inner.asset_folder.path, "Textures", cubemap.get_name() + ".hdr")
                )
...
def export_landscape(context, landscape_actor):
    ...
    with Usd.StageCacheContext(cache):
        ...
        if context.options.inner.asset_options.use_payload:
            ...
            asset_prim.GetPayloads().AddPayload(mesh_path)
            ...
            mat_proxy_prim.GetReferences().AddReference(mat_path)
