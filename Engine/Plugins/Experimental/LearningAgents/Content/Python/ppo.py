...
from train_common import check_array, check_network, check_network_gradients, Profile
...
class PPOTrainer:
    ...
    def compute_advantages_and_returns(self, obs, obs_next, mem, mem_next, rew, terminated, truncated, warn_about_advantages=True):
        ...
        if warn_about_advantages and adv.std() < 1e-8:
            logger.warning('Advantages standard deviation very small: %f!' % adv.std())
        ...
        if warn_about_advantages and np.all(adv == 0.0):
            logger.warning('All advantages were zero - no better than average actions found! Critic may be under-trained.')
    ...
    def train_policy(self, mask, obs, act, mod, mem, adv, logp, policy_window):
        ...
        for i in range(0, policy_window):
            ...
            ratio = schema_log_prob(self.action_schema, act_dist, act[:,i], mod[:,i]) - logp[:,i]
            ...
            loss_ent += self.action_ent_weight * (mask[:,i] * -(schema_entropy(self.action_schema, act_dist, mod[:,i]) / self.act_num)).mean() / policy_window
    ...
    def extract_from_buffer(self, buffer):
        ...
        mod = torch.as_tensor(buffer['mod'], device=self.policy.device)
        ...
        if np.all(rew == 0.0): logger.warning('All rewards were zero!')
        if np.all(terminated | truncated): logger.warning('All episodes were zero length!')
    
        return (obs, obs_next, act, mod, mem, mem_next, rew, terminated, truncated)
    ...
    def train(
        ...
        with Profile('PPO log prob'):
        
            logp = self.compute_old_logp(obs, mem, act, mod)
        
        with Profile('PPO create batches'):
